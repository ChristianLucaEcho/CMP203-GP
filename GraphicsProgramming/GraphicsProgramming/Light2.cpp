...
void Light2::handleInput(float dt)
{
	// Handle user input
}

void Light2::update(float dt)
{
	// update scene related variables.

	rotation += speed * dt;
	float radius = 10;
	lightPosition.x = sin(rotation) *radius;
	lightPosition.z = cos(rotation) * radius;
	// Calculate FPS for output
	calculateFPS();
}

void Light2::PushVertex(Vertex v) {
...
void Light2::drawCircle(GLfloat x, GLfloat y, GLfloat z, GLfloat radius, GLint numberOfSides)
{
...
void Light2::drawTriangle(Vertex v1, Vertex v2, Vertex v3) {
...
void Light2::drawSquare(Vertex v1, Vertex v2, Vertex v3, Vertex v4) {
	glBegin(GL_QUADS);
	PushVertex(v1);
	PushVertex(v2);
	PushVertex(v4);
...
/*void Light2::drawCube(Vertex v1, Vertex v2, Vertex v3, Vertex v4, Vertex v5, Vertex v6, Vertex v7, Vertex v8) {
	glBegin(GL_QUADS);
...
void Light2::render() {
	...
	glLoadIdentity();
	// Set the camera
	gluLookAt(0, 16, 36, 0, 0, 0, 0, 1, 0);

	GLfloat Light_Ambient[] = { 0.3f, 0.3f, 0.3f, 1.0f };
	GLfloat Light_Diffuse[] = { 1.0f, 1.0f, 1.0f, 1.0f };
	GLfloat Light_Position[] = { lightPosition.x, 0.0f, lightPosition.z, 1.0f };
	...
	glMaterialfv(GL_FRONT, GL_SPECULAR, mat_specular);
	glMateriali(GL_FRONT, GL_SHININESS, low_shininess);
	glMaterialfv(GL_FRONT, GL_EMISSION, mat_diff_red);
	gluSphere(gluNewQuadric(), 1.0, 40, 40);
	glPopMatrix();

	glPushMatrix();
	glTranslatef(lightPosition.x, 0.0, lightPosition.z);
	glColor3f(1, 2, 0);
...
void Light2::resize(int w, int h)
{
	width = w;
	height = h;
...
void Light2::displayText(float x, float y, float r, float g, float b, char* string) {
	// Get Lenth of string
	int j = strlen(string);
